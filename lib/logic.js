...
async function executionDeposit(accountTransfer) {
  ...
  accountTransfer.contract.balance += accountTransfer.clientValue + accountTransfer.supplierValue
  ...
  const escrowContractRegistryClient = await getAssetRegistry(ESCROW_CONTRACT_REG)
  await escrowContractRegistryClient.update(accountTransfer.contract)
}
...
async function executionDepositSupplierAndDriver(accountTransfer) {
  ...
  accountTransfer.supplier.balance -= accountTransfer.supplierValue
  accountTransfer.contract.balance += accountTransfer.driverValue + accountTransfer.supplierValue

  const accountRegistryClient = await getAssetRegistry(ACC_ASSET_REG)
  await accountRegistryClient.update(accountTransfer.driver)
  await accountRegistryClient.update(accountTransfer.supplier)
  const escrowContractRegistryClient= await getAssetRegistry(ESCROW_CONTRACT_REG)
  await escrowContractRegistryClient.update(accountTransfer.contract)
}
...
async function createAds(client, data){
  ...
  var ads = factory.newResource(NS, ADS_NS, uuidv4());
  ads.owner = client;
...
async function supplierAcceptAds(supplier, ads, data){
  //Create AdsContract
  var factory = getFactory()
  var adsContract = factory.newResource(NS, ADS_CONTRACT_NS, uuidv4());
