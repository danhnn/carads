...
async function AdsPreparationDeposit(accountTransfer) {
  ...
  const accountRegistryClient = await getAssetRegistry("org.carads.Client")
  await accountRegistryClient.update(accountTransfer.owner)
  
  const accountRegistrySupplier = await getAssetRegistry("org.carads.Supplier")
  await accountRegistrySupplier.update(accountTransfer.supplier)
}